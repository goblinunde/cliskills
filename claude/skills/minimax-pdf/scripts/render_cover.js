...
function loadPlaywright() {
  ...
  const platformHint =
    os.platform() === "linux"
      ? "On Linux, ensure nodejs/npm are installed. Fedora example: sudo dnf install -y nodejs npm && npm install -g playwright && npx playwright install chromium"
  ...
  try {
    ...
    if (r.status !== 0) {
      console.error(JSON.stringify({
        ...
        hint: os.platform() === "linux"
          ? "On Linux, run npm install -g playwright && npx playwright install chromium after nodejs/npm are available"
