...
For Fedora-based Codex workspaces, install the system packages first, then the Python and Node dependencies:

```bash
sudo dnf install -y python3 python3-pip nodejs npm
python3 -m pip install --user reportlab pypdf matplotlib
...
If Playwright still cannot launch Chromium, fix the host browser runtime first; the PDF cover pipeline depends on a working local Playwright browser.
...
- `bash scripts/make.sh fix` installs Python packages and Playwright, but it does not install missing Fedora system packages
- cover fonts are fetched at render time, so offline cover rendering falls back to system fonts
