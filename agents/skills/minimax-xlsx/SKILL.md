...
```bash
sudo dnf install -y python3 python3-pip libreoffice
python3 minimax-skills/skills/minimax-xlsx/scripts/libreoffice_recalc.py --check
```

On Fedora, use `python3` consistently for every helper script. Dynamic validation depends on a working local LibreOffice install.
...
Fedora note:
- install LibreOffice from `dnf` before relying on Tier 2 validation
- run `python3 SKILL_DIR/scripts/libreoffice_recalc.py --check` before promising recalculation support
