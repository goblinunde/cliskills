...
print_platform_fix_hint() {
  local distro
  distro="$(detect_linux_distro)"
  case "$distro" in
    macos)
      yellow "  macOS hint: brew install python3 node npm"
      ;;
    fedora|rhel|centos|rocky|alma)
      yellow "  Fedora hint: sudo dnf install -y python3 python3-pip nodejs npm"
      ;;
    ubuntu|debian|linuxmint|pop)
      yellow "  Debian/Ubuntu hint: sudo apt-get install -y python3 python3-pip nodejs npm"
      ;;
    *)
      yellow "  Install python3, python3-pip, nodejs, and npm with your system package manager."
      ;;
  esac
}
...
cmd_fix() {
  ...
  # Python packages
  if command -v python3 &>/dev/null; then
    if python3 -m pip install --break-system-packages -q reportlab pypdf matplotlib 2>/dev/null \
      || python3 -m pip install -q reportlab pypdf matplotlib 2>/dev/null; then
      green "  ✓ Python packages installed (reportlab, pypdf, matplotlib)"
    else
      yellow "  pip install failed — try: python3 -m pip install --user reportlab pypdf matplotlib"
      rc=3
    fi
  else
    yellow "  python3 not found — cannot install Python dependencies automatically"
    print_platform_fix_hint
    rc=2
  fi
