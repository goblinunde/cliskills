...
| **Chinese font** | Noto Sans CJK SC on Fedora/Linux, Microsoft YaHei on Windows |
...
```bash
sudo dnf install -y python3 python3-pip nodejs npm google-noto-sans-cjk-fonts
python3 -m pip install --user "markitdown[pptx]"
mkdir -p slides/output slides/imgs
cd slides
npm init -y
npm install pptxgenjs react-icons react react-dom sharp
```

For Codex on Fedora, prefer local `npm install ...` inside the slide workspace. `require("pptxgenjs")` does not reliably resolve from a global install.
...
5. Fonts: Chinese = Noto Sans CJK SC on Fedora/Linux or Microsoft YaHei on Windows; English = Arial (or approved alternative)
...
- `python3 -m pip install --user "markitdown[pptx]"` — text extraction
- `npm install pptxgenjs` — creating from scratch in the current slide workspace
- `npm install react-icons react react-dom sharp` — icons and image helpers (optional)
