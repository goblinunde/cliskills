...
- `uvx` available on the local machine
...
On Fedora, make sure `uvx` is already on `PATH` before adding the server, then restart Codex after editing `~/.codex/config.toml`.

**Step 3:** After configuration, tell the user to restart their app and verify that the MiniMax MCP server appears in the client's MCP/status view.
