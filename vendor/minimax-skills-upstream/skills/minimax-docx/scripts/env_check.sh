...
if command -v pandoc &>/dev/null; then
    ...
    case "$OS" in
        ...
        linux|wsl)
            case "$DISTRO_ID" in
                fedora|rhel|centos|rocky|alma) echo "           Install: sudo dnf install -y pandoc" ;;
                ubuntu|debian|linuxmint|pop)  echo "           Install: sudo apt-get install -y pandoc" ;;
                *)                            echo "           Install: pandoc from your distro package manager" ;;
            esac
            ;;
...
if command -v soffice &>/dev/null; then
    ...
    if ! $soffice_found; then
        ...
        case "$OS" in
            ...
            linux|wsl)
                case "$DISTRO_ID" in
                    fedora|rhel|centos|rocky|alma) echo "           Install: sudo dnf install -y libreoffice" ;;
                    ubuntu|debian|linuxmint|pop)  echo "           Install: sudo apt-get install -y libreoffice" ;;
                    *)                            echo "           Install: libreoffice from your distro package manager" ;;
                esac
                ;;
...
if [ -n "$current_lang" ] && echo "$current_lang" | grep -qi "utf-8\|utf8"; then
    ...
    case "$DISTRO_ID" in
        fedora|rhel|centos|rocky|alma)
            echo "           Fedora system-wide: sudo localectl set-locale LANG=en_US.UTF-8"
            ;;
    esac
fi
