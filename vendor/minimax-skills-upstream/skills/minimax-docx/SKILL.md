...
**Fedora quick setup:**
```bash
sudo dnf install -y dotnet-sdk-8.0 pandoc libreoffice zip unzip liberation-fonts google-noto-sans-cjk-fonts
bash scripts/env_check.sh
```

On Fedora/Linux, prefer `Noto Sans CJK SC` for local preview and editing. Templates that reference `Microsoft YaHei` still open, but they render through font substitution unless Microsoft fonts are installed separately.
