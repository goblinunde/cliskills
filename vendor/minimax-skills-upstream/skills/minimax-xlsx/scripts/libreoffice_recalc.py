...
def find_soffice() -> str | None:
    ...
    candidates = [
        ...
        "/usr/lib64/libreoffice/program/soffice",                # Fedora/RHEL
        "/usr/lib/libreoffice/program/soffice",                  # Debian/Ubuntu
        "/usr/bin/soffice",
        "/usr/bin/libreoffice",
...
def detect_linux_distro() -> str | None:
    """Return Linux distro id from /etc/os-release when available."""
...
def install_hints() -> list[str]:
    """Return distro-aware install hints for LibreOffice."""
    distro = detect_linux_distro()
    hints = ["  macOS:  brew install --cask libreoffice"]
    if distro in {"fedora", "rhel", "centos", "rocky", "alma"}:
        hints.append("  Fedora: sudo dnf install -y libreoffice")
    elif distro in {"ubuntu", "debian", "linuxmint", "pop"}:
        hints.append("  Linux:  sudo apt-get install -y libreoffice")
    else:
        hints.append("  Linux:  install libreoffice with your distro package manager")
    return hints
