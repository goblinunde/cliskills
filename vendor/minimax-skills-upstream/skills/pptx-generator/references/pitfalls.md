...
```bash
python3 -m markitdown output.pptx | grep -iE "xxxx|lorem|ipsum|placeholder|this.*(page|slide).*layout"
```
...
1. Generate slides -> Extract text with `python3 -m markitdown output.pptx` -> Review content
...
```bash
python3 -m markitdown slide-XX-preview.pptx
```
